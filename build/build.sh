...
function InstallDotNetCliProjectJson {
  DotNetCliProjectJsonVersion="$( GetVersionsPropsVersion DotNetCliProjectJsonVersion )"
  DotNetInstallVerbosity=""

  DotNetRoot="$RepoRoot/artifacts/.dotnet/$DotNetCliProjectJsonVersion"
  DotNetInstallScript="$DotNetRoot/dotnet-install.sh"
  ...
  # Install a stage 0
  SdkInstallDir="$DotNetRoot/sdk/$DotNetCliProjectJsonVersion"

  if [ ! -d "$SdkInstallDir" ]
  then
    bash "$DotNetInstallScript" --version $DotNetCliProjectJsonVersion --install-dir $DotNetRoot $DotNetInstallVerbosity
    LASTEXITCODE=$?
...
function Build {
  ...
  if [ -z $solution ]
  then
    solution="$RepoRoot/cli-migrate.sln"
  fi

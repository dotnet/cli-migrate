...
function InstallDotNetCliProjectJson {
  $DotNetCliProjectJsonVersion = GetVersionsPropsVersion -Name "DotNetCliProjectJsonVersion"
  $DotNetInstallVerbosity = ""

  $DotNetRoot = Join-Path $RepoRoot "artifacts\.dotnet\$DotNetCliProjectJsonVersion"
  $DotNetInstallScript = Join-Path $DotNetRoot "dotnet-install.ps1"
  ...
  # Install a stage 0
  $SdkInstallDir = Join-Path $DotNetRoot "sdk\$DotNetCliProjectJsonVersion"

  if (!(Test-Path $SdkInstallDir)) {
    # Use Invoke-Expression so that $DotNetInstallVerbosity is not positionally bound when empty
    Invoke-Expression -Command "$DotNetInstallScript -Version $DotNetCliProjectJsonVersion -InstallDir $DotNetRoot $DotNetInstallVerbosity"
...
function Build {
  ...
  if ($solution -eq "") {
    $solution = Join-Path $RepoRoot "cli-migrate.sln"
  }
